...
from kamae.spark.transformers import ListSumTransformer
...
class TestListSum:
    ...
    def listwise_transform_df_sort_desc(self, spark_session):
        return spark_session.createDataFrame(
            [
                (1, 1, 1, 12.0),  # should be ignored in top3 desc
                (1, 2, 2, 12.0),
                (1, 2, 3, 12.0),
                (1, 8, 4, 12.0),
            ],
    ...
    def listwise_transform_df_sort_asc(self, spark_session):
        return spark_session.createDataFrame(
            [
                ...
                (1, 2, 2, 9.0),
                (1, 2, 3, 9.0),
                (1, 8, 4, 9.0),  # should be ignored in top3 asc
            ],
            [
                "search_id",
                "value_col",
    ...
    def test_spark_sum_transform_with_sort(
        self,
        input_dataframe,
        value_col,
        sort_col,
    ...
    def test_spark_sum_transform_with_segmentation(
        self,
        input_dataframe,
        value_col,
        segment_col,
    ...
    def test_spark_sum_transform_raises_without_query_id_col(
        self, listwise_transform_df_no_filter
    ):
        # given
        transformer = ListSumTransformer(
            inputCol="value_col",
        ...
        with pytest.raises(ValueError):
            transformer.transform(listwise_transform_df_no_filter.drop("expected"))
    ...
    def test_list_sum_transform_spark_tf_parity(
        self,
        spark_session,
        list_size,
        qid_tensor,
