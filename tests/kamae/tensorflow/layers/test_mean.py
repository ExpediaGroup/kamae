...
from kamae.tensorflow.layers import MeanLayer


class TestMean:
    @pytest.mark.parametrize(
        "inputs, input_name, mean_constant, input_dtype, output_dtype, expected_output",
        [
            (
                ...
                None,
                None,
                tf.constant([1.5, 2.0, 2.5], dtype="float32"),
            ),
            (
            ...
            (
                tf.constant([[[2.0, 10.0, 30.0]]]),
                "input_3",
                3.0,
                "float64",
                None,
                tf.constant([[[2.5, 6.5, 16.5]]], dtype="float64"),
            ),
            (
                [
                    tf.constant([[7.0], [4.0], [3.0]]),
                    tf.constant([[1.0], [2.0], [3.1]]),
                    tf.constant([[8.0], [2.0], [1.0]]),
    ...
    @pytest.mark.parametrize(
        "inputs, input_name, mean_constant",
        [
            (
    ...
    def test_mean_raises_error(self, inputs, input_name, mean_constant):
        with pytest.raises(ValueError):
            layer = MeanLayer(name=input_name, mean_constant=mean_constant)
            layer(inputs)
    ...
    def test_mean_with_bad_types_raises_error(
