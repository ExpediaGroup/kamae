...
class TestModulo:
    @pytest.mark.parametrize(
        ...
        [
            (
                tf.constant([5, -3, 11], dtype="int32"),
                "input_1",
                5.0,
                None,
                None,
                tf.constant([0, 2, 1], dtype="int32"),
            ),
            (
            ...
            (
                tf.constant([[["4.0", "2.0", "32.0"]]]),
                "input_3",
                3.0,
                "float64",
                "float32",
                tf.constant([[[1.0, 2.0, 2.0]]]),
            ),
            (
                [
            ...
            (
                ...
                "float64",
                None,
                tf.constant([0.7700009346008301], dtype="float64"),
            ),
        ],
    )
    def test_modulo(
    ...
    def test_modulo_raises_error(self, inputs, input_name, divisor):
        with pytest.raises(ValueError):
            layer = ModuloLayer(name=input_name, divisor=divisor)
            layer(inputs)
    ...
    def test_modulo_with_bad_types_raises_error(
