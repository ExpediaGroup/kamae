...
"""Multi-backend input validation utilities for backend-agnostic layers."""
...
def is_tensor(x: Any) -> bool:
    """
    Checks if the input is a Keras tensor (backend-agnostic).

    Uses keras.ops.is_tensor() which works across all backends.

    :param x: Input to check
    :returns: True if x is a Keras tensor
    """
    ...
    def _enforce_single_tensor_input(
        ...
        **kwargs: Any,
        ...
        if is_tensor(inputs):
            ...
            if len(input_list) == 1 and is_tensor(input_list[0]):
    ...
    def _enforce_multiple_tensor_input(
        ...
        **kwargs: Any,
        ...
        if is_tensor(inputs):
            ...
            input_list = list(iter_values(inputs))
            if len(input_list) > 1 and all([is_tensor(inp) for inp in input_list]):
    ...
    def _allow_single_or_multiple_tensor_input(
        ...
        **kwargs: Any,
        ...
        if is_tensor(inputs):
            ...
            input_list = list(iter_values(inputs))
            if all([is_tensor(inp) for inp in input_list]):
                processed_inputs = input_list
            else:
                raise ValueError(
