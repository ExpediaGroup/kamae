...
class SegmentMinTransformer(
    ...
    """
    Calculate the listwise minimum across the query id and segment column combinations.
    ...
    def _transform(self, dataset: DataFrame) -> DataFrame:
        """
        Calculate the min, segmented based on the second input column.
        ...
        dataset = dataset.withColumn(
            self.getOutputCol(),
            F.min(F.col(val_col_name)).over(window_spec),
        )
    ...
    def get_tf_layer(self) -> tf.keras.layers.Layer:
        """
        Gets the tensorflow layer for the segment min transformer.

        :returns: Tensorflow keras layer with name equal to the layerName parameter that
         performs an averaging operation.
        """

        return SegmentMinLayer(
