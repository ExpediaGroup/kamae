...
from kamae.tensorflow.layers import SegmentMaxLayer

from .base import BaseTransformer


class SegmentMaxTransformer(
    BaseTransformer,
    MultiInputSingleOutputParams,
    ListwiseStatisticsParams,
):
    """
    Calculate the listwise maximum across the query id and segment column combinations.

    Example: calculate the maximum price of rates within rooms in the same query.
    ...
    def _transform(self, dataset: DataFrame) -> DataFrame:
        """
        Calculate the max, segmented based on the second input column.
        ...
        dataset = dataset.withColumn(
            self.getOutputCol(),
            F.max(F.col(val_col_name)).over(window_spec),
        )
    ...
    def get_tf_layer(self) -> tf.keras.layers.Layer:
        """
        Gets the tensorflow layer for the segment max transformer.

        :returns: Tensorflow keras layer with name equal to the layerName parameter that
         performs an averaging operation.
        """

        return SegmentMaxLayer(
