...
class SegmentMaxLayer(BaseLayer):
    """
    Calculate the max of the first passed tensor, segmented by the values of the second passed tensor.

    Example: calculate the max price of rates within rooms in the same query.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Initializes the Segment Max layer.
    ...
    def _call(self, inputs: Iterable[Tensor], **kwargs) -> Tensor:
        """
        Calculate the max of the first input, segmented by the values of the second input.

        :param inputs: The iterable tensors for the features.
        :returns: Thew new tensor result column.
        """

        def per_batch_max(v, s):
        ...
        return tf.map_fn(
            lambda x: per_batch_max(x[0], x[1]),
            (values, segments),
